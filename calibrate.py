...
def main(argv):
    language = 'chinese'
    infile = 'D:\\svn\\python\\process\\NN.NAWE.2010.343'
    parfile = 'D:\\svn\\python\\process\\NN.NAW.txt'
    outfile = 'D:\\svn\\python\\process\\out.txt'
    figfile = 'D:\\svn\\python\\process\\demo.png'

    try:
        opts, args = getopt.getopt(argv,"hi:p:o:f:l:",["infile","parfile","outfile","figfile","language"])
    except getopt.GetoptError:
        print('python calbrate.py -i <data infile> -p <par infile> -o <txt outfile> -f <figure outfile) -l <chinese|english> ')
        sys.exit(2)
    for opt, arg in opts:
        if opt in ('-h','--help'):
            print('python calbrate.py -i <data infile> -p <par infile> -o <txt outfile> -f <figure outfile) -l <chinese|english>  ')
            ...
            outfile = arg
        elif opt in ("-f", "--figfile"):
            figfile = arg
            filetype = figfile.split('.')[-1].lower()
            #print("filename = %s, ext=%s" %(figfile,filetype))
            if (filetype!='png' and filetype!='pdf'):
                figfile += '.png'
        elif opt in ('-l' '--language'):
            if (arg[0]=='e' or arg[0] == 'E'):
                language = 'english'

    #print('Cal Input=%f%s. Cal Sensitivity=%fm/s**2/%s, Digitizer Sensitivity=%fCt/V' %(cal_input,nCalMode,cal_stvt,nCalMode,ad_stvt))
    #cal_input, cal_stvt, ad_stvt,
    print('input file:  %s' % infile)
    print('par file:  %s'   % parfile)
    print('figure file: %s' % figfile)
