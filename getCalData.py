...
# =================================================================
def CalAccPulse(st0, outname, fCalIn=1.0, fCalStvt=1.0, fADStvt=1.0, CalMode='V', language='chinese', nSaveMode=0,
                pdf=None):
    sps = st0.stats.sampling_rate
    nSPS = int(sps + 0.5)
    npts = st0.stats.npts
...
#    outname  -- 输出文件名
def CalVelPulse(st0, outname, fCalIn=1.0, fCalStvt=1.0, fADStvt=1.0, CalMode='V', language='chinese', nSaveMode=0,
                pdf=None):
    # 获取 SEED 数据
    sps = st0.stats.sampling_rate
    nSPS = int(sps + 0.5)
...
def doCalPulse(st0, figfile, outfile, nNetMode, nCalMode, cal_stvt, pulse_input, ad_stvt, m=0, language='chinese',
               nSaveMode=0, pdf=None):
    if (nNetMode == 'V'):
...
def DoAllCal(infile, parfile, figfile_0, outfile, language='chinese'):
    # 解析数据文件
    (SinOrPulse, nNetMode, nCalMode, cal_stvt, cal_input, ad_stvt, fTm0, fStart, fEnd, fGain, fFreq) = parserPar(
        parfile)
    if (nNetMode == 'ERR'):
        ...
        if (nSaveMode == 0):  # pdf mode
            with PdfPages(figfile_0) as pdf:
                for m in range(nCh):
                    (fFreq, cal_gain) = doCalSin(st[m], figfile_0, outfile,
                                                 nNetMode, nCalMode, cal_stvt[m], cal_input, ad_stvt, fTm0, fStart,
                                                 fEnd, fGain, fFreq,
            ...
            for m in range(nCh):
                ...
                (fFreq, cal_gain) = doCalSin(st[m], figfile, outfile,
                                             nNetMode, nCalMode, cal_stvt[m], cal_input, ad_stvt, fTm0, fStart, fEnd,
                                             fGain, fFreq,
